...
int main(int argc, char **argv) {
    ...
    if (tcc_compile_string(s_first, first_code) == -1) {
    	printf("Failed to compile\n");
    	return 1;
    }
	if (tcc_relocate(s_first, TCC_RELOCATE_AUTO) == -1) 	{
    ...
    if (tcc_compile_string(s_second, second_code) == -1) {
    	printf("Failed to compile\n");
    	return 1;
    }
	if (tcc_relocate(s_second, TCC_RELOCATE_AUTO) == -1) {
    	printf("Failed to relocate\n");
    	return 1;
    }
	...
	tcc_set_extended_symtab_callbacks(s_third, NULL,
		&my_lookup_by_name, &my_sym_used, &my_data);
    if (tcc_compile_string(s_third, third_code) == -1) {
    	printf("Failed to compile\n");
    	return 1;
    }
	if (tcc_relocate(s_third, TCC_RELOCATE_AUTO) == -1) {
    	printf("Failed to relocate\n");
    	return 1;
    }
